...
public class GestionAvancee {
    ...
    public static List<String> listeMembresGroupes(String groupEmail) throws IOException, GeneralSecurityException {
        ...
        try {
            ...
            if (mem != null) {
                mem.stream().filter((m) -> (m.getRole().equals("MEMBER"))).forEach((m) -> {
                    l.add(m.getEmail());
                });
    ...
    public static String listeGroupesFavoris() throws IOException, GeneralSecurityException {
        ...
        try {
            ...
            mess = groups.stream().filter((group) -> (group.getEmail().indexOf("favoris-", 0) == 0)).map((group) -> group.getEmail() + " - " + group.getName() + " - " + group.getDirectMembersCount() + "\n").reduce(mess, String::concat);
        } catch (IOException | GeneralSecurityException exception) {
